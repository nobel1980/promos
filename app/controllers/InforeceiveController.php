                ...
                foreach($electionarea as $ea)
                {
                    if($ea['division'] == $divkey)
                    {
                        ...
                        if($ea['recinfo'] == 'send' || $ea['recinfo'] == 'resend')
                        {
                            ...
                            */
                            $html .= "<td><a href='inforeceive/recorrej/".$ea['recinfoid']."'  title='প্রতিবেদন গ্রহণ অথবা বর্জন করুন'><i title='প্রতিবেদন গ্রহণ অথবা বর্জন করুন' class='flaticon-viewing'></i></a></td>";
                        }
                        ...
                        {
                            $html .= "<td><a href='inforeceive/view/".$ea['recinfoid']."' title='প্রতিবেদন পুনরায় প্রদানের জন্য প্রেরণ করা হয়েছে '><i class='flaticon-share10'></i></a></td>";
                        }
                        else if($ea['recinfo'] == 'received')
                        {
                            $html .= "<td><a href='inforeceive/view/".$ea['recinfoid']."'  title='প্রতিবেদন দেখুন'><i class='flaticon-affirmative1'></i></a></td>";
                        }
                        ////////////////////////////////////
...
//$html = '<a href="'.$url.'" title="'.$title.'" onclick=alert("asdasd")';
        /*
    ...
    public function inforeceivedAction()
    {
		$this->view->disable();
        $msg = null;
        ...
        if (($this->request->isPost()) && ($this->request->isAjax() == true)) {
			...
			if (!$information->save()) {
				$msg = $information->getMessages();//"error2";
    ...
    public function infobackAction()
    {
        ...
        if (($this->request->isPost()) && ($this->request->isAjax() == true)) {
            $id = $this->request->getQuery("id", "int");
            $comment = $this->request->getQuery("comment", "string");
            $information = Informations::findFirstById($id);
            ...
            if (!$information->save()) {
                $msg =  $this->flash->error($information->getMessages());
            } else {
                $msg = "success";
            }
        }
    ...
    public function recorrejAction($id)
    {
        $information = Informations::findFirstById($id);
