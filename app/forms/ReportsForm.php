...
class ReportsForm extends Form
{
	public function initialize($entity=null, $options=null)
	{
        ...
        if($user_info['profileid'] == '6')
        {
            $dd = new InformationsController();
            $user_info = $dd->getUserInfo(); // "district=$district_code"
            $ea = Electionarea::findFirst("id=".$user_info['electionarea']);

            $this->add(new Hidden('district',array('value'=>$ea->district)));
            $this->add(new Hidden('division',array('value'=>$ea->division)));
            $this->add(new Hidden('electionarea',array('value'=>$ea->id)));
        }
        else if($user_info['profileid'] == '4')
        {
            $dd = new InformationsController();
            $user_info = $dd->getUserInfo(); // "district=$district_code"
            $dis = Zilla::findFirst("zillaid=".$user_info['district']);

            $this->add(new Hidden('district',array('value'=>$dis->zillaid)));
            $this->add(new Hidden('division',array('value'=>$dis->divid)));

            $this->add(new Select('electionarea', Electionarea::find("district=".$dis->zillaid), array(
                'using' => array('id', 'title_bn'),
                'useEmpty' => true,
                'emptyText' => '...',
                'emptyValue' => ''
        ...
        else if($user_info['profileid'] == '5')
        {
            $dd = new InformationsController();
            $user_info = $dd->getUserInfo(); // "district=$district_code"
            $div = $user_info['division'];

            $this->add(new Select('division', Division::find("divid=".$div), array(
                'using' => array('divid', 'divname'),
                'useEmpty' => true,
                'emptyText' => '...',
                'emptyValue' => '',
